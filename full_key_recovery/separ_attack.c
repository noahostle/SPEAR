...
static void display_format_phase(const char *phase, char *out, size_t out_len)
{
    unsigned stage = 0u;
    uint64_t idx = 0u;
    size_t j = 0u;
    ...
    if (sscanf(phase, "stage%u-iv", &stage) == 1u) {
        snprintf(out, out_len, "K%u_IV", stage);
        return;
    }
    if (sscanf(phase, "stage%u-rot", &stage) == 1u) {
        snprintf(out, out_len, "K%u_ROT", stage);
        return;
    }
    if (sscanf(phase, "stage%u-cycles", &stage) == 1u) {
        snprintf(out, out_len, "K%u_CYCLE", stage);
        return;
    }
    if (sscanf(phase, "stage%u-reps", &stage) == 1u) {
        snprintf(out, out_len, "K%u_REPS", stage);
        return;
    }
    if (sscanf(phase, "k8-search-%" SCNu64, &idx) == 1u) {
        snprintf(out, out_len, "K8_SEARCH");
        return;
    }
    for (size_t i = 0u; phase[i] != '\0' && j + 1u < out_len; i++) {
        unsigned char ch = (unsigned char)phase[i];
        if (ch == '-') ch = '_';
        out[j++] = (char)((ch >= 'a' && ch <= 'z') ? (ch - ('a' - 'A')) : ch);
    }
    out[j] = '\0';
}
...
static void display_commit_stage(uint8_t stage, KeyPair pair)
{
    if (!g_live.compact_enabled) return;
...
static void display_commit_stage1(KeyPair pair)
{
    if (!g_live.compact_enabled) return;
            ...
            for (uint32_t c = 0u; c < 16u; c++) {
                for (uint32_t d = 0u; d < 16u; d++) {
                    ...
                    for (uint32_t x = 0u; x < 16u; x++) {
                        uint8_t t = separ_sbox2((uint8_t)(x ^ a));
                        t = separ_sbox2((uint8_t)(t ^ b));
                        t = separ_sbox2((uint8_t)(t ^ c));
                        t = separ_sbox2((uint8_t)(t ^ d));
                        t = (uint8_t)(separ_sbox2((uint8_t)(t ^ a ^ b)) ^ c ^ d);
...
static void print_full_recovery_summary(const InwardProbeResult *result)
{
    ...
    ts_printf("K7=(%04X,%04X)", result->pairs[7].k0, result->pairs[7].k1);
    ts_printf("K6=(%04X,%04X)", result->pairs[6].k0, result->pairs[6].k1);
    ts_printf("K5=(%04X,%04X)", result->pairs[5].k0, result->pairs[5].k1);
    ts_printf("K4=(%04X,%04X)", result->pairs[4].k0, result->pairs[4].k1);
    ts_printf("K3=(%04X,%04X)", result->pairs[3].k0, result->pairs[3].k1);
    ts_printf("K2=(%04X,%04X)", result->pairs[2].k0, result->pairs[2].k1);
    ts_printf("K1=(%04X,%04X)", result->pairs[1].k0, result->pairs[1].k1);
    if (!g_debug_output) {
        display_mark_done();
        display_finish();
                                            ...
                                            uint16_t *current)
{
    ...
    *out_ctx = build_full_context(key_words, iv_words, NULL, 0u);
                                  ...
                                  StagePairHitVec *hits)
{
    ...
    if (!g_debug_output && hits->count > 0u) {
        display_set_best_iv_words(hits->rows[0].sample_iv);
    }
                                      ...
                                      InwardProbeResult *out)
{
    ...
    for (size_t i = 0u; i < hits.count; i++) {
        ...
        if (!g_debug_output) {
            display_commit_stage(target_stage, hits.rows[i].pair);
        }
...
int main(int argc, char **argv)
{
    ...
    if (!g_debug_output) {
        display_set_best_iv_words(result.iv_words);
        display_commit_stage(7u, result.pairs[7]);
        display_commit_stage(6u, result.pairs[6]);
        display_commit_stage(5u, result.pairs[5]);
        display_commit_stage(4u, result.pairs[4]);
        display_commit_stage(3u, result.pairs[3]);
        display_commit_stage(2u, result.pairs[2]);
        display_commit_stage1(result.pairs[1]);
        display_mark_done();
        display_render();
        display_finish();
        putchar('\n');
