...
static void display_reset_values(void)
{
    ...
    snprintf(g_live.branch, sizeof(g_live.branch), "-");
    snprintf(g_live.progress, sizeof(g_live.progress), "-");
    display_set_unknown(g_live.k8, sizeof(g_live.k8));
    display_set_unknown(g_live.k7, sizeof(g_live.k7));
    display_set_unknown(g_live.s8, sizeof(g_live.s8));
    display_set_unknown(g_live.k6, sizeof(g_live.k6));
    display_set_unknown(g_live.s7, sizeof(g_live.s7));
    display_set_unknown(g_live.k5, sizeof(g_live.k5));
    display_set_unknown(g_live.s6, sizeof(g_live.s6));
}
...
static void display_render(void)
{
    ...
    snprintf(header, sizeof(header),
             "%-13s | %-6s | %-17s | %-9s | %-9s | %-4s | %-9s | %-4s | %-9s | %-4s",
             "phase", "branch", "prog", "K8", "K7", "s8", "K6", "s7", "K5", "s6");
    snprintf(values, sizeof(values),
             "%-13s | %-6s | %-17s | %-9s | %-9s | %-4s | %-9s | %-4s | %-9s | %-4s",
             g_live.phase, g_live.branch, g_live.progress,
             g_live.k8, g_live.k7, g_live.s8, g_live.k6, g_live.s7, g_live.k5, g_live.s6);
...
static void display_set_branch(size_t current, size_t total)
{
    if (!g_live.compact_enabled) return;
    if (total == 0u) snprintf(g_live.branch, sizeof(g_live.branch), "-");
    else snprintf(g_live.branch, sizeof(g_live.branch), "%zu/%zu", current, total);
    display_render();
}
...
static void display_commit_outer_stage(uint8_t stage, KeyPair pair, uint16_t state_word)
{
    if (!g_live.compact_enabled) return;
    if (stage == 7u) {
        display_format_pair(g_live.k7, sizeof(g_live.k7), pair);
        display_format_state(g_live.s8, sizeof(g_live.s8), state_word);
    } else if (stage == 6u) {
        display_format_pair(g_live.k6, sizeof(g_live.k6), pair);
        display_format_state(g_live.s7, sizeof(g_live.s7), state_word);
    } else if (stage == 5u) {
        display_format_pair(g_live.k5, sizeof(g_live.k5), pair);
        display_format_state(g_live.s6, sizeof(g_live.s6), state_word);
    }
    display_render();
}
...
static void display_apply_outer_summary(KeyPair k8, KeyPair k7, uint16_t s8, KeyPair k6, uint16_t s7, KeyPair k5, uint16_t s6)
{
    ...
    display_format_pair(g_live.k7, sizeof(g_live.k7), k7);
    display_format_state(g_live.s8, sizeof(g_live.s8), s8);
    display_format_pair(g_live.k6, sizeof(g_live.k6), k6);
    display_format_state(g_live.s7, sizeof(g_live.s7), s7);
    display_format_pair(g_live.k5, sizeof(g_live.k5), k5);
    display_format_state(g_live.s6, sizeof(g_live.s6), s6);
    display_render();
}
                                                   ...
                                                   uint16_t states_a[9])
{
    ...
    for (int stage = 7; stage >= 5; stage--) {
        ...
        if (!g_debug_output) {
            display_commit_outer_stage((uint8_t)stage, best.pair, best.state_word);
            if (stage == 7) {
                                       ...
                                       KeyPair *out_pair)
{
    ...
    for (size_t i = 0u; i < beam_count; i++) {
        ...
        if (!opt->debug_output) {
            plain_printf("[hybrid-k8] iv %zu/%zu row=%u iv=%s", i + 1u, beam_count, beam[i].row_score, iv_hex);
        }
    ...
    if (!opt->debug_output) {
        plain_printf("[hybrid-k8] winner=(%04X,%04X) aggregate_score=%" PRIu64 " hits=%u",
                     agg[0].pair.k0, agg[0].pair.k1, agg[0].aggregate_score, agg[0].hits);
    }
    *out_pair = agg[0].pair;
    free(agg);
...
static void usage(const char *argv0)
{
    fprintf(stderr,
            "usage:\n"
            "  %s full [--workers N] [--iv HEX32] [--oracle-key HEX64] [--debug]\n"
            "  %s full --search-trials N [--search-beam N] [--search-seed N] [--workers N] [--oracle-key HEX64] [--debug]\n"
            "  %s full --force-k8 K0,K1 [--workers N] [--iv HEX32] [--oracle-key HEX64] [--debug]\n",
            argv0, argv0, argv0);
}
...
static size_t run_outer_attack_once(const Options *opt)
{
    ...
    for (size_t i = 0u; i < boot_count; i++) {
        ...
        if (!g_debug_output) {
            display_set_branch(i + 1u, boot_count);
    ...
    if (!g_debug_output) {
        if (success_count > 0u) {
            display_set_branch(summary_branch, boot_count);
        ...
        plain_printf("K8=%s K7=%s s8=%s K6=%s s7=%s K5=%s s6=%s",
                     g_live.k8, g_live.k7, g_live.s8, g_live.k6, g_live.s7, g_live.k5, g_live.s6);
    }
    if (opt->validate) {
...
int main(int argc, char **argv)
{
    ...
    if (opt.search_trials > 0u) {
        if (run_outer_attack_beam_hybrid(&opt) == 0u) {
            fatal("no searched IV survived the combined public K8..K5 attack");
        }
        return 0;
    }
    if (run_outer_attack_once(&opt) == 0u) {
        fatal("no outer branch survived public recursion");
    }
    return 0;
}
